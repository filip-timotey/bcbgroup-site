...
function syncRoleLabel(profile){
  const cards=document.querySelectorAll('.bcb-admin-user-card');
  cards.forEach(card=>{
    const label=card.querySelector('span');
    if(label) label.textContent=isOwnerProfile(profile)?'Owner':profile?.role==='admin'?'Administrator':'Editor';
    card.classList.toggle('is-owner',isOwnerProfile(profile));
  });
}
...
function ensureFleetSafetyStyles(){if(document.querySelector('link[data-fleet-safety-styles]'))return;const link=document.createElement('link');link.rel='stylesheet';link.href='../css/admin-fleet-safety.css';link.dataset.fleetSafetyStyles='true';document.head.appendChild(link);}
