...
function syncRoleLabel(profile){const cards=document.querySelectorAll('.bcb-admin-user-card');cards.forEach(card=>{const label=card.querySelector('span');if(label)label.textContent=isOwnerProfile(profile)?'Owner':profile?.role==='admin'?'Administrator':'Editor';card.classList.toggle('is-owner',isOwnerProfile(profile));});}
