...
function setBusy(isBusy) {
  ...
  saveButton.innerHTML = isBusy ? '<i class="fa-solid fa-circle-notch fa-spin"></i> Se salvează…' : '<i class="fa-solid fa-floppy-disk"></i> Salvează proiectul';
}
...
async function saveProject(event) {
  ...
  if (!title) return showMessage("Titlul proiectului este obligatoriu.", "error");
  ...
  if (error) {
    console.error(error);
    return showMessage("Nu am putut salva proiectul. Încearcă din nou.", "error");
  }

  showMessage(publicStatus ? "Proiect salvat și publicat. Va apărea automat pe site." : "Proiect salvat ca schiță și ascuns de pe site.");
  if (pageTitle) pageTitle.textContent = title;
  if (heading) heading.textContent = title;
  if (mediaPanel) mediaPanel.hidden = false;
  window.history.replaceState({}, "", `${window.location.pathname}?id=${encodeURIComponent(projectId)}`);
  await loadMedia();
}
...
async function loadMedia() {
  ...
  const { data: media, error } = await supabase.from("project_media").select("id, media_type, storage_path, title, is_cover, created_at").eq("project_id", projectId).order("created_at", { ascending: true });
  if (error) {
  ...
  mediaGrid.innerHTML = items.map((item) => {
    ...
    const preview = item.media_type === "video" ? `<video src="${escapeHtml(url)}" muted playsinline controls preload="metadata"></video>` : `<img src="${escapeHtml(url)}" alt="Media proiect" loading="lazy">`;
    return `<article class="bcb-media-card" data-media-id="${escapeHtml(item.id)}" data-storage-path="${escapeHtml(item.storage_path)}">
      ...
      <div class="bcb-media-card-footer">
        ${item.media_type === "image" ? `<button class="bcb-media-cover ${item.is_cover ? "is-active" : ""}" type="button" data-action="cover">${item.is_cover ? "Copertă" : "Setează copertă"}</button>` : '<span>Video</span>'}
...
async function uploadFiles(files) {
  ...
  for (const file of files) {
    if (file.size > 500 * 1024 * 1024) { failed += 1; continue; }
    ...
    const { error: uploadError } = await supabase.storage.from("project-media").upload(storagePath, file, { cacheControl: "3600", upsert: false, contentType: file.type || undefined });
    if (uploadError) { console.error(uploadError); failed += 1; continue; }

    const { error: rowError } = await supabase.from("project_media").insert({ project_id: projectId, media_type: mediaType, storage_path: storagePath, title: file.name, uploaded_by: currentUser.id });
    if (rowError) { console.error(rowError); failed += 1; continue; }
    uploaded += 1;
  }

  mediaInput.value = "";
  mediaInput.disabled = false;
  uploadStatus.textContent = failed ? `${uploaded} încărcate, ${failed} nereușite.` : `${uploaded} fișier(e) încărcate cu succes.`;
  await loadMedia();
}
...
async function removeMedia(card) {
  ...
  if (!mediaId || !storagePath || !window.confirm("Ștergi definitiv acest fișier?")) return;

  const { error: storageError } = await supabase.storage.from("project-media").remove([storagePath]);
  if (storageError) return showMessage("Fișierul nu a putut fi șters.", "error");

  const { error } = await supabase.from("project_media").delete().eq("id", mediaId);
  if (error) return showMessage("Înregistrarea media nu a putut fi ștearsă.", "error");

  await supabase.from("projects").update({ cover_path: null, updated_by: currentUser.id }).eq("id", projectId).eq("cover_path", storagePath);
  await loadMedia();
}
...
async function setCover(card) {
  ...
  await supabase.from("project_media").update({ is_cover: false }).eq("project_id", projectId);
  const { error: mediaError } = await supabase.from("project_media").update({ is_cover: true }).eq("id", mediaId);
  const { error: projectError } = await supabase.from("projects").update({ cover_path: storagePath, updated_by: currentUser.id }).eq("id", projectId);
  if (mediaError || projectError) return showMessage("Coperta nu a putut fi setată.", "error");
  await loadMedia();
}
...
(async function init() {
  if (await requireStaffSession()) await loadProject();
})();
