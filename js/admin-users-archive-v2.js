...
function renderArchivePanel() {
  ...
  if (!panel) return;
  if (!isOwnerProfile(currentProfile)) { panel.hidden = true; return; }
  panel.hidden = false;
  ...
  <p style="margin:0 0 18px;color:var(--bcb-muted,#667085);line-height:1.6">Conturile arhivate nu mai au acces la Business Manager. Datele și istoricul operațional sunt păstrate, iar Owner-ul poate restaura accesul în siguranță.</p>
  <div class="bcb-users-list">${archivedProfiles.length ? archivedProfiles.map(profile => {
    ...
    return `<article class="bcb-user-row is-archived" data-archived-user-id="${esc(profile.id)}"><div class="bcb-user-main"><div class="bcb-user-avatar">${esc(initials(identifier))}</div><div><h3>${esc(identifier)}</h3><p>${esc(profile.email || `ID · ${profile.id.slice(0, 8)}…`)}</p><div class="bcb-user-badges"><span class="bcb-user-badge"><i class="fa-solid fa-box-archive"></i> ARHIVAT</span></div></div></div><div>${profile.role === "admin" ? "Administrator" : "Editor"}</div><div class="bcb-user-status"><span>Arhivat · ${esc(archivedAt)}</span></div><div class="bcb-user-actions"><button type="button" class="bcb-user-save" data-restore-user="${esc(profile.id)}"><i class="fa-solid fa-user-check"></i> Restaurează accesul</button></div></article>`;
...
function alignUi() {
  ...
  usersList.querySelectorAll("[data-user-id]").forEach(row => {
    if (archivedIds.has(row.dataset.userId)) { row.remove(); return; }
    const button = row.querySelector('button[data-action="delete"]');
    if (!button) return;
    const label = button.classList.contains("bcb-user-danger") ? "Elimină accesul" : "Solicită eliminarea";
    if (button.textContent?.trim() !== label) button.innerHTML = `<i class="fa-solid fa-user-lock"></i> ${label}`;
  });
...
async function loadArchived() {
  ...
  loading = true;
  try {
    const { data, error } = await supabase.from("profiles").select("id,full_name,email,role,is_active,is_owner,is_archived,archived_at,archived_by,created_at").eq("is_archived", true).order("archived_at", { ascending:false });
...
function scheduleRefresh() {
  ...
  refreshTimer = window.setTimeout(loadArchived, 80);
}
...
(async function initArchiveUx() {
  ...
  await loadArchived();
  if (usersList) new MutationObserver(() => { alignUi(); scheduleRefresh(); }).observe(usersList, { childList:true, subtree:true });
  const approvals = document.querySelector("#bcb-owner-approvals");
  if (approvals) new MutationObserver(alignUi).observe(approvals, { childList:true, subtree:true });
})();
