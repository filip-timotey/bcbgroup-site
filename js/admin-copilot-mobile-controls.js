const POSITION_KEY="bcb-ai-mobile-position-v1";
...
function clampWidgetPosition(widget,left,top){
  ...
  const width=rect.width||58,height=rect.height||58;
  return {
...
function savePosition(widget){
  if(!isMobile())return;
  const rect=widget.getBoundingClientRect();
  try{localStorage.setItem(POSITION_KEY,JSON.stringify({left:Math.round(rect.left),top:Math.round(rect.top)}));}catch{}
}
...
function setupDrag(widget,toggle){
  ...
  toggle.addEventListener("pointerdown",event=>{
    if(!isMobile()||getPanel()?.classList.contains("is-open"))return;
...
function initialize(){
  ...
  if(widget.dataset.mobileControls==="1")return true;
  widget.dataset.mobileControls="1";
